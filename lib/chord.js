 ...
 * @returns {Chord | null} - The chord representation of the voicing
 */
export function voicingToChord(voicing, intervalToFingerOptions = () => ({}), stringIntervals = GUITAR_STANDARD_TUNING_INTERVALS) {
  ...
  for (const [stringNumber, [stringInterval, chordInterval]] of enumerate(zip(stringIntervals, intervalDistance))) {
    currentIntervalOffset += stringInterval;
    if (chordInterval == null)  {
      continue;
    }

    chord.push([reverseString(stringNumber + 1), chordInterval - currentIntervalOffset, intervalToFingerOptions(voicing[stringNumber])]);
  }
 ...
 * @param {[Interval | null, Interval | null, Interval | null, Interval | null, Interval | null, Interval | null]} voicing 
 * @returns {Array<[Chord]>} - Array of chords with all inversions
 */
export function getInversions(voicing) {
