...
/**
 * Apply drop 3 + 2 voicing to a set of intervals.
 * @param {Array<Interval>} intervals 
 * @returns {Array<Interval>} - The intervals after applying drop 3 + 2 voicing
 */
function drop3and2(intervals) {
  const clone = [...intervals];
  moveItem(clone, -3, 0); // drop 3
  moveItem(clone, -2, 0); // drop 2
  return clone;
}

/**
 * Apply close voicing to a set of intervals.
 * @param {Array<Interval>} intervals 
 * @returns {Array<Interval>} - The intervals after applying close voicing
 */
...
export const ALLOWED_VOICING_5 = ["CLOSE", "DROP_2", "DROP_3", "DROP_2_AND_3", "DROP_2_AND_4", "DROP_3_AND_2"];
